module.exports = class BossHealthPlugin {
  constructor(omegga, config, store) {
    this.omegga = omegga;
    this.config = config;
    this.store = store;
    this.bossTeamNames = config["boss-team-names"].map(s => s.toLowerCase());

    this.minis = [];
  }

  async getPawn({controller}) {
    const pawnRegExp = new RegExp(`(?<index>\\d+)\\) BP_PlayerController_C .+?PersistentLevel\\.${controller}\\.Pawn = (?:None|BP_FigureV2_C'.+?:PersistentLevel\\.(?<pawn>BP_FigureV2_C_\\d+)')?$`);
    this.omegga.writeln(`GetAll BP_PlayerController_C Pawn Name=${controller}`);
    const pawnMatch = await this.omegga.addWatcher(pawnRegExp, {timeoutDelay: 100});
    return pawnMatch[0].groups.pawn;
  }

  async getPawnHealth(pawn) {
    const damageRegExp = new RegExp(`(?<index>\\d+)\\) BP_FigureV2_C .+?PersistentLevel\\.${pawn}\\.Damage = (?<damage>[\\d.-]+)$`);
    const damageLimitRegExp = new RegExp(`(?<index>\\d+)\\) BP_FigureV2_C .+?PersistentLevel\\.${pawn}\\.DamageLimit = (?<limit>[\\d.-]+)$`);

    this.omegga.writeln(`GetAll BP_FigureV2_C Damage Name=${pawn}`);
    const damageMatch = await this.omegga.addWatcher(damageRegExp, {timeoutDelay: 100});

    this.omegga.writeln(`GetAll BP_FigureV2_C DamageLimit Name=${pawn}`);
    const damageLimitMatch = await this.omegga.addWatcher(damageLimitRegExp, {timeoutDelay: 100});
  ...
  displayHealth(teamName, playerName, [health, maxHealth], toPlayers) {
    const frac = Math.max(0, health) / maxHealth;

    // display the preliminary info
    this.omegga.broadcast(`<color="c00"><b>${teamName} Health</></> <color="900">(${playerName})</>`);

    // display the healthbar
    const barLength = this.config["health-bar-size"];
    const healthSize = Math.round(frac * barLength);
    const message = `<color="aaa">[<color="0a0">${"=".repeat(healthSize)}</><color="a00">${"=".repeat(barLength - healthSize)}</>]</> <b>${Math.round(Math.max(0, health))}</><color="aaa">/</>${Math.round(maxHealth)}`;
  ...
  async loop() {
    ...
    for (const mg of minigames) {
      const bossTeam = mg.teams.find(t => this.bossTeamNames.includes(t.name.toLowerCase()));
      ...
      if (mini.boss.pawn) {
        ...
        if (!pawn || pawn != mini.boss.pawn)
          mini.boss = {name: null, pawn: null, controller: null, health: null};
      }
      ...
      const timeoutPassed = now - mini.announce.last >= 1000 * this.config["announce-timeout"];
      const healthChangePassed = Math.abs(mini.announce.lastHealthFrac - healthFrac) >= this.config["announce-health-change"];

      if (this.config["announce-require-time-and-health-change"]) {
        // both time and health must be satisfied
        if (timeoutPassed && healthChangePassed)
          shouldAnnounce = true;
      } else {
      ...
      if (shouldAnnounce) {
        ...
        this.displayHealth(mini.bossTeam.name, mini.boss.name, mini.boss.health, mg.members.map(m => m.name));
      }
    }
  }

  async init() {
    setInterval(this.loop.bind(this), this.config["interval"]);
  }
